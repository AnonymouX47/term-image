...
class ImageCLI(object):
    ...
    def parse_arguments(self):
        ...
        for index, argument in enumerate(sys.argv[1:]):
            ...
            if not is_valid_parameter:
                self.create_error_message(f"Invalid parameter: {argument}", "Use -- in the beginning of key")
    ...
    def draw_image(self, filename, parameters):
        ...
        if not size:
            self.create_error_message(f"Invalid size parameter: {parameters.get('size')}")
        execute_function = lambda: None
        if os.path.isfile(filename):
            execute_function = lambda: DrawImage(filename, size)
            return
        else:
            execute_function = lambda: DrawImage.from_url(filename, size)
        try:
            execute_function()
