...
class Renderable(metaclass=RenderableMeta, _base=True):
    ...
    **and** its subclasses (i.e :term:`descendant` attributes) which should be exported
    ...
    def _get_render_data_(self, *, iteration: bool) -> RenderData:
        ...
        Should include **copies** of any **variable/mutable** internal/external state
        required for rendering and other data generated from constant state but which
        should **persist** throughout a render operation.

        May also be used to "allocate" storage for mutable/variable data specific to a
        render operation (which may involve consecutive/repeated renders of one or more
        frames).
        ...
        * return the same :py:class:`~term_image.renderable.RenderData` instance.
        ...
        NOTE:
            * The base implementation initializes all render data not defined by
              :py:class:`~term_image.renderable.Renderable` to the value ``None``.
              These should be updated by the respective subclasses that defined them,
              if neccessary.
