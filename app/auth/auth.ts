  ...
  (request: Request, response: Response): void => {
    ...
    axios({
      ...
      .then((axiosResponse: AxiosResponse<any>): void => {
        response.cookie(
          "githubToken",
          Buffer.from(axiosResponse.data.access_token, "utf8").toString(
            "base64"
          )
        );
...
authRouter.get("/get/current/user", (request: Request, response: Response) => {
  const githubClient = new Octokit({
    auth: Buffer.from(request.cookies.githubToken, "base64").toString("utf8"),
  });
  githubClient
    .request("GET /user")
    .then((githubResponse: any): void => {
